...
# Comparative Genomic and Proteomic Analysis of Geoduck clam (Panopea generosa)

- Geoduck Genome already assembled in *Putnum et al. 2022*
- Geoduck Transcritome -- genome linked transcriptome [link here](https://github.com/RobertsLab/code/tree/master/r_projects/sam/20230328-pgen-gene_annotation-update)
...
## download other transcriptome from clam species
```{r, cache=TRUE}
...
#write.csv(pgenerosa_simple_goslims, file="/Users/oliviacattau/Documents/GitHub/olivia-geoduck/output/MASTERgenelistPGEN.csv")
```
...
# Oyster female gonad comparison
## load data from Dheilly et al. 2012 from Supplemental Table 3 and filter for genebank accession numbers
```{r}
...
## load results from BLASTn from Dheilly 2012 paper fasta against Pgen *transcriptome*
...
## Filter by genes present in female gonad
...
# Reproductive genes by gonad
```{r}
gonad_reproductive<-pgenerosa_long %>%
 filter(tissue=="gonad")%>%
  filter(binary==1) 
  

result <- grep("reproductive", gonad_reproductive$Term, ignore.case = TRUE)

matching_rows <- gonad_reproductive[row.names(gonad_reproductive) %in% result, ]
  
#write.csv(matching_rows, file="/Users/oliviacattau/Documents/GitHub/olivia-geoduck/output/gonad_reproductive_Genelist.csv")
kable(matching_rows)
```
...
heatmap7<- ggplot(tissues2, aes(x=tissues, y=Term_unique, fill=n, text=n))+geom_tile()+theme_bw()+ scale_fill_gradient2(low=("green"),mid="black", high=("red"), midpoint=17.5)+theme(axis.text.x = element_text(angle = 45, hjust = 1, size=10, face="bold"))+theme(axis.text.y=element_text(size=10))+labs(y="Biological GO Terms")

Tis_normal_2<-Tis_normal %>%
  filter(Term_unique != "NA")

heatmap8<-ggplot(Tis_normal_2, aes(tissue, y=Term_unique, fill=normalized))+geom_tile()+theme_bw()+ scale_fill_gradient2(low = "green", mid = "black", high = "red", midpoint = 1)+theme(axis.text.x = element_text(angle = 45, hjust = 1, size=10, face="bold"))+theme(axis.text.y=element_text(size=10))+labs(y="Biological GO Terms", x="RNA-seq Library", fill="value")

heatmap8b<-ggplot(Tis_normal_2, aes(x=Term_unique, y=tissue, fill=normalized))+geom_tile()+theme_bw()+ scale_fill_gradient2(low = "green", mid = "black", high = "red", midpoint = 1)+theme(axis.text.x = element_text(angle = 75, hjust = 1, size=10, face="bold"))+theme(axis.text.y=element_text(size=10))+labs(y="Biological GO Terms")

heatmap9<-ggplot(Tis_normal_2, aes(tissue, y=Term_unique, fill=log_scale))+ geom_tile()+theme_bw()+scale_fill_gradient2(low = "green", mid = "black", high = "red", midpoint = 0)+theme(axis.text.x = element_text(angle = 45, hjust = 1, size=10, face="bold"))+theme(axis.text.y=element_text(size=10))+labs(y="Biological GO Terms")

species_normal_2<-species_normal %>%
  filter(Term_unique != "NA")%>%
  filter(transcriptome == "Manila_r" | transcriptome == "Mercenaria_r")

heatmap10<-ggplot(species_normal_2, aes(transcriptome, y=Term_unique, fill=log(normalized)))+geom_tile()+scale_fill_gradient2(low = "green", mid = "black", high = "red", midpoint = 0, na.value = "grey50")+theme_bw()+labs(y="Biological GO Terms")+theme(axis.text.x = element_text(angle = 45, hjust = 1, size=10, face="bold"))+theme(axis.text.y=element_text(size=10))
```
...
gonad_count_pie<-gonad_count_unique[-1,]
na_zero2<-na_zero[1:20,]
...
pie_geoduck_R<-pie(na_zero2$n.Pgenerosa, labels = na_zero2$Term_unique, main="Geoduck Gene Library w/ Biological GO Terms")

pie_geoduck_gonad<-pie(gonad_count_pie$n, labels=gonad_count_pie$Term_unique, main="Geoduck gonad RNA-seq library",cex = 2)

genome_sizes<-read.csv(file="/Users/oliviacattau/Documents/GitHub/olivia-geoduck/data/genome_size.csv")

# Remove a column
genome_sizes$X <- NULL

genome_sizes<-genome_sizes[-c(6),]

genome_sizes_ranked<-genome_sizes %>% 
  arrange(desc(size))

genome_sizes_bar<-ggplot(data=genome_sizes_ranked, aes(x=species, y=size))+geom_bar(stat="identity", fill="#A4A4A4", color="black")+theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1, size=10, face="italic"))+theme(axis.text.y=element_text(size=10))+ylab("Percent Coverage of P. generosa Gene List")+geom_text(aes(label = size), vjust = -0.3)+
  scale_x_discrete(limits=genome_sizes_ranked$species)
```
