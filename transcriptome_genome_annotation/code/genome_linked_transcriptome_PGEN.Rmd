...
```{r, cache=TRUE}
mappingMatrix<-read.csv(file="https://gannet.fish.washington.edu/Atumefaciens/20220419-pgen-gene_annotation_mapping/20220419-pgen-gene-accessions-gene_id-gene_name-gene_description-alt_gene_description-go_ids.tab", header=FALSE, sep = '\t', fill=TRUE)
colnames(mappingMatrix) <- c("target_id", "identifiers", "gene_id", "gene_name", "annotation", "alt_annotation", "goterms")
#14,672
presence_count <- sum(!is.na(mappingMatrix[, 7]) & mappingMatrix[, 7] != "") #14,254
```
...
## load data and filter for genebank accession numbers
```{r}
...
## load results from blast from genebank list of accession number above against Pgen *transcriptome*
...
names(cgigas_gonad_pgen)[1]<-"Cgigas_GeneBank"
names(cgigas_gonad_pgen)[12]<-"e-value"

#can add goslims, don't see the value right now (5.24.23)
```
...
stacked_barplot<-ggplot(top20terms, aes(x=transcriptome, y=n, fill=Term_unique))+geom_bar(stat = "identity")+theme_bw()+scale_fill_manual(values=cols)

stacked_barplot2<-ggplot(compairative_transcriptomes, aes(x=transcriptome, y=n, fill=Term_unique))+geom_bar(stat = "identity")+theme_bw()+scale_fill_manual(values=cols)
...
heatmap7<- ggplot(tissues2, aes(x=tissues, y=Term_unique, fill=n, text=n))+geom_tile()+theme_bw()+ scale_fill_gradient(low="#c2f9ff", high="#003460")+theme(axis.text.x = element_text(angle = 45, hjust = 1) )
...
ggplotly(heatmap7, tooltip="next")
heatmap8
heatmap9
heatmap10
```
...
#Plot the pie using base R
pie_geoduck_R<-pie(na_zero$n.Pgenerosa, labels = na_zero$Term_unique, main="Geoduck gene library GoSlims")
```
# scatter plots for e-values
```{r, cache=TRUE}
