...
genome_sizes$species <- factor(genome_sizes$species, levels=unique(genome_sizes$species))

genome_sizes_bar<-ggplot(data=genome_sizes, aes(x=species, y=size))+geom_bar(stat="identity", fill="#A4A4A4", color="black")+theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1, size=10, face="italic"))+theme(axis.text.y=element_text(size=10))+ylab("Percent Coverage of P. generosa Transcriptome")
```
