...
pgenerosa_complete<-salmonMatrix %>% 
  left_join(goslims, by=c("target_id")) 
...
gonad_genes<-left_join(cgigas_gonad_pgen, goslims, by="gene")
head(gonad_genes)
```
...
```{r}
gonad_genes2<-gonad_genes %>%
  filter(gonad_genes$`e-value` != 0)%>%
  filter(!is.na(alt_gene_description))%>%
  left_join(gonad_genes2, cgigas_gonad_pgen, by="Cgigas_GeneBank")

head(gonad_genes2) 
```
103 genes with gene descriptions 

### STATE HERE
need to run blast again with Dheilly results
